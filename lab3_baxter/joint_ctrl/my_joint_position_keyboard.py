...
def map_keyboard(side):
    ...
    theta={}
    theta['right_j0'] = np.float32(raw_input("Enter joint1: "))
    theta['right_j1'] = np.float32(raw_input("Enter joint2: "))
    theta['right_j2'] = np.float32(raw_input("Enter joint3: "))
    theta['right_j3'] = np.float32(raw_input("Enter joint4: "))
    theta['right_j4'] = np.float32(raw_input("Enter joint5: "))
    theta['right_j5'] = np.float32(raw_input("Enter joint6: "))
    theta['right_j6'] = np.float32(raw_input("Enter joint7: "))
    
    limb = intera_interface.Limb(side)
    joints = limb.joint_names()

    def judge(dict):
        if (abs(limb.joint_angle(joints[0])-theta['right_j0'])>0.1 or
                abs(limb.joint_angle(joints[1])-theta['right_j1'])>0.1 or
                abs(limb.joint_angle(joints[2])-theta['right_j2'])>0.1 or
                abs(limb.joint_angle(joints[3])-theta['right_j3'])>0.1 or
                abs(limb.joint_angle(joints[4])-theta['right_j4'])>0.1 or
                abs(limb.joint_angle(joints[5])-theta['right_j5'])>0.1 or
                abs(limb.joint_angle(joints[6])-theta['right_j6'])>0.1):
            return True
        else:
            return False
    ...
    def set_j(limb, joint_name, theta):
        # current_position = limb.joint_angle(joint_name)
        joint_command = {joint_name: theta}
        limb.set_joint_positions(joint_command)

    while(judge(theta)):
        #print("setting joint position\n")
        # set_j(limb, joints[0], theta['right_s0'])
        # set_j(limb, joints[1], theta['right_s1'])
        # set_j(limb, joints[2], theta['right_e0'])
        # set_j(limb, joints[3], theta['right_e1'])
        # set_j(limb, joints[4], theta['right_w0'])
        # set_j(limb, joints[5], theta['right_w1'])
        # set_j(limb, joints[6], theta['right_w2'])
    ...
    def set_j_new(limb, joint_name, delta):
        current_position = limb.joint_angle(joint_name)
