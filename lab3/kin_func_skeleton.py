...
def rotation_3d(omega, theta):
    ...
    #YOUR CODE HERE
    norm = np.sqrt(np.sum(np.square(omega)))
    w_skew = skew_3d(omega)
    rot = np.identity(3) + (w_skew/norm)*np.sin(norm*theta) + (np.matmul(w_skew,w_skew)/norm**2)*(1 - np.cos(norm*theta))
    return rot
...
def homog_2d(xi, theta):
    ...
    #YOUR CODE HERE
    ang = xi[2]*theta
    rot = np.array([[np.cos(ang),-np.sin(ang)],[np.sin(ang),np.cos(ang)],[0,0]])
    #print(rot)
    p1 = np.array([[1-np.cos(ang),np.sin(ang)],[-np.sin(ang),1-np.cos(ang)]])
    #print(p1)
    p2 = np.array([[0,-1],[1,0]])
    #print(p2)
...
def homog_3d(xi, theta):
    ...
    e_omegahat = rotation_3d(omega,theta)

    norm = np.sqrt(np.sum(np.square(omega)))
    w_skew = skew_3d(omega)
    v = np.array([[xi[0]],[xi[1]],[xi[2]]])
    #print(v)
    upper_right1_1 = np.matmul(w_skew,v)
    upper_right1_2 = np.matmul(np.identity(3) - e_omegahat,upper_right1_1)
    
    upper_right2_1 = omega
    #print(upper_right2_1)
    upper_right2_2 = np.matmul(upper_right2_1,v)*theta
    upper_right = (1/norm**2)*(upper_right1_2 + upper_right2_2)
    #print(upper_right)
    g = np.hstack((e_omegahat,upper_right))
...
def prod_exp(xi, theta):
    ...
    g = np.identity(4)
    for j in range(num):
        g_part = homog_3d(xi[:,j],theta[j])
        g = np.matmul(g,g_part)
    return g
