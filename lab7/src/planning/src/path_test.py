...
def main():
    ...
    executor = Controller(Kp,Kd,Ki,Kw,Limb('right'))
    ...
    # planner.add_box_obstacle(size, "table", box_pose)

    # #wall
    # size = np.array([0.4,0.1,1.2])
    # box_pose = PoseStamped()
    # box_pose.header.frame_id = "base"
    # box_pose.pose.position.x = 0.5
    ...
    # box_pose.pose.orientation.z = 0.0
    # box_pose.pose.orientation.w = 1.0
    # planner.add_box_obstacle(size, "wall", box_pose)
    # ##
    ...
    while not rospy.is_shutdown():
        ...
        while not rospy.is_shutdown():
            try:
                ...
                if not executor.execute_path(plan):
